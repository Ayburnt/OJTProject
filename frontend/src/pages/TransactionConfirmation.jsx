...
export default function Ticket() {
    ...
    const { transactCode } = useParams();
    const handlePrint = useReactToPrint({
        contentRef: componentRef,
        documentTitle: `${transactCode}-booking-confirmation`,
    });
    ...
    useEffect(() => {
        api.get(`/attendees/transaction-info/${transactCode}/`)
            .then(res => {
                setAttendee(res.data);
                setEventDetails(res.data[0].event_details);
                                    ...
                                    <div className="bg-gray-300 border-b border-dashed px-4 sm:px-6 py-3 sm:py-4 rounded-t-2xl flex flex-row items-center justify-between flex-wrap">
                                        ...
                                        <div className="text-right mt-2 sm:mt-0">
                                            ...
                                            <p className="text-sm sm:text-lg font-outfit font-semibold text-secondary">
                                                {row.ticket_read?.ticket_name} Access
                                            </p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="order-1 lg:order-2 flex-1 p-4 sm:p-6 flex flex-col items-center justify-start">
                                            <div className="flex flex-col items-center w-full">
                                                <div className="w-32 sm:w-40 h-32 sm:h-40 bg-gray-200 flex items-center justify-center rounded-lg">
                                                    <img src={row.ticket_qr_image} className="w-full" alt="" />
                                                </div>
                                                ...
                                                <p className="text-xs font-outfit sm:text-sm text-gray-400 mt-2">
                                                    Purchased on {formatPurchaseDate(row.created_at)}
                                                </p>
                                            </div>
                                        </div>
                                            ...
                                            <div className="space-y-4 text-xs sm:text-sm">
                                                ...
                                                <div className="flex flex-col border-b pb-3">
                                                    ...
                                                    <div className="text-right">
                                                        <p className="font-semibold text-lg">{row.ticket_read?.price === 0 || row.ticket_read?.price === '0.00' ? 'Free' : row.ticket_read?.price}</p>
                                                    </div>
                                                </div>
                                                {/* Rules */}
                                                <ul className="font-outfit text-gray-500 list-disc list-inside space-y-1 text-xs sm:text-sm">
                ...
                {Array.isArray(attendee) && attendee.length > 0 && (
                    attendee.map((row) => (
                        <>
                            ...
                            <div key={row.id} className="w-[93%] border-2 border-dashed border-[#D9D9D9] rounded-xl">
                                ...
                                <div className="border-2 border-[#D9D9D9] rounded-lg shadow-md overflow-hidden text-center font-outfit w-full">
                                    ...
                                    <div className="bg-Dark-grayish-blue font-semibold font-outfit text-white text-xl py-3">
                                        <p>{row.ticket_read.ticket_name}</p>
                                    </div>
                                </div>
                            </div>
                        </>
