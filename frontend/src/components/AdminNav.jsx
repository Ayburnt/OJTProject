...
function AdminNav() {
  ...
  // ✅ Frontend-only route → title mapping
  const pageTitles = {
    "/admin-dashboard": "Dashboard Overview",
    "/admin-eventcontrol": "Event Control Overview",
    "/admin-attendees": "Attendees",
    "/admin-review": "Admin Review",
  };

  const title = pageTitles[currentPath] || "Admin Dashboard";

  // ✅ Update browser tab title on route change
  useEffect(() => {
    document.title = `${title} | Sari-Sari Events`;
  }, [title]);
    ...
    <>
          ...
          <div className="bg-white w-full h-full rounded-xl shadow-lg p-6 relative overflow-y-auto">
            ...
            <div className="flex flex-col items-center justify-center">
              <ul className="w-full text-center bg-white text-gray-800 flex flex-col items-start">
                <Link
                  to="/admin-dashboard"
                  className={`${title === "Dashboard Overview" ? "bg-[#009494]" : ""} w-full text-start cursor-pointer hover:text-teal-600 transition-colors`}
                >
                  <p
                    className={`${title === "Dashboard Overview" ? "bg-[#EEEEEE]" : ""} py-3 ml-5 pl-3 font-outfit`}
                  >
                    Dashboard
                  </p>
                </Link>
                ...
                <Link
                  ...
                  className={`${title === "Event Control Overview" ? "bg-[#5BD4D4]" : ""} w-full text-start cursor-pointer hover:text-teal-600 transition-colors`}
                >
                  <p
                    className={`${title === "Event Control Overview" ? "bg-[#EEEEEE]" : ""} py-3 ml-5 pl-3 font-outfit`}
                  >
                    Event Control
                  </p>
                </Link>
                ...
                <Link
                  to="/admin-attendees"
                  className={`${title === "Attendees" ? "bg-[#FF965D]" : ""} w-full text-start cursor-pointer hover:text-teal-600 transition-colors`}
                >
                  <p
                    className={`${title === "Attendees" ? "bg-[#EEEEEE]" : ""} py-3 ml-5 pl-3 font-outfit`}
                  >
                    Attendees
                  </p>
                </Link>
                ...
                <Link
                  to="/admin-review"
                  className={`${title === "Admin Review" ? "bg-[#EF4B4C]" : ""} w-full text-start cursor-pointer hover:text-teal-600 transition-colors`}
                >
                  <p
                    className={`${title === "Admin Review" ? "bg-[#EEEEEE]" : ""} py-3 ml-5 pl-3 font-outfit`}
                  >
                    Admin Review
                  </p>
                </Link>
            ...
            <div className="flex flex-col mt-10 gap-3 pl-4 text-secondary">
              ...
              >
                ...
                <span className="text-sm font-outfit">Home</span>
              </div>
              <button
                className="flex items-center gap-2 hover:underline font-outfit"
                onClick={logout}
              >
      ...
      <div className="hidden col-span-2 md:flex md:flex-col md:justify-between md:h-screen md:w-full md:bg-white md:p-6 md:shadow z-40">
        ...
        <div className="flex flex-col gap-2 text-left text-secondary">
          <div
            className="flex items-center gap-2 mb-24 cursor-pointer"
            onClick={() => navigate('/')}
          >
            <IoIosArrowBack className="text-xl" />
            <span className="text-sm font-outfit">Home</span>
          ...
          <button
            className="flex items-center gap-2 hover:underline font-outfit mb-7"
           onClick={logout}
          >
            <IoExitOutline className="text-xl transform -scale-x-100" />
            <span className="text-sm">Log out</span>
